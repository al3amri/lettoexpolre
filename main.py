...
class TikTok:
    def __init__(self):
        ...
        try:
            self.amount = int(input('> Enter the Amount of shares You want: '))
        except ValueError:
            self.close('Value error, expected an integer as (1000)')
    ...
    def close(self, message):
        print(f'\n{message}')
        os.system("title letto's TikTok SharesBot - Restart required")
        os.system('pause >NUL')
        os.system("title letto's TikTok SharesBot - Closing...")
        sleep(3)
        os._exit(0)
    ...
    def update_title(self):
        ...
        while self.added < self.amount:
            ...
            os.system(
                f"title letto's TikTok SharesBot - Added: {self.added} Shares/{self.amount} Shares "
                f'({round(((self.added / self.amount) * 100), 3)}%) ^| Active Threads: '
                f'{threading.active_count()} ^| Time Remaining: {time_remaining}'
            )
            sleep(0.2)
        os.system(
            f"title letto's TikTok SharesBot - Added: {self.added}/{self.amount} "
            f'({round(((self.added / self.amount) * 100), 3)}%) ^| Active Threads: '
...
if __name__ == '__main__':
    os.system("cls && title letto's TikTok SharesBot")
    main = TikTok()
    main.start()
